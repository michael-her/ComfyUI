...
function sjsxCategoryPathExtractor(nodeDef: ComfyNodeDefImpl): string[] {
  const category = nodeDef.category || ''
  const categoryParts = category
    ? category.split(/[/\\]/).filter(Boolean)
    : []
  const path =
    categoryParts[0]?.toLowerCase() === 'sjsx'
      ? categoryParts.slice(1)
      : categoryParts
  return [...path, nodeDef.name]
}
...
function isSjsxNode(node: ComfyNodeDefImpl): boolean {
  const category = node.category || ''
  return (
    node.name.startsWith('Sjsx_') ||
    category.startsWith('sjsx/') ||
    category.startsWith('sjsx\\')
  )
}
...
export const DEFAULT_TAB_ID = 'sjsx' as const
...
class NodeOrganizationService {
  ...
  organizeNodesByTab(
    ...
    switch (tabId) {
      case 'sjsx':
        return this.organizeSjsx(nodes)
      case 'essentials':
  ...
  private organizeSjsx(nodes: ComfyNodeDefImpl[]): NodeSection[] {
    const sjsxNodes = nodes.filter(isSjsxNode)
    if (sjsxNodes.length === 0) return []

    return [
      {
        tree: unwrapTreeRoot(
          buildNodeDefTree(sjsxNodes, {
            pathExtractor: sjsxCategoryPathExtractor
          })
        )
      }
    ]
