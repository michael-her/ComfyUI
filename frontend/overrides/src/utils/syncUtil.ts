...
function isWorkflowSjsxCompanionPath(filePath: string): boolean {
  const normalized = filePath.replace(/\\/g, '/')
  return (
    normalized.toLowerCase().endsWith('.sjsx') &&
    (normalized === 'workflows' || normalized.startsWith('workflows/'))
  )
}
...
export async function syncEntities<T>(
  ...
  const files = (await api.listUserDataFullInfo(dir))
    ...
    .filter((file) => !isWorkflowSjsxCompanionPath(file.path))
