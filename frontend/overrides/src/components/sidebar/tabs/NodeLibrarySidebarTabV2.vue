    ...
    <template #body>
      ...
      <div class="flex h-full flex-col">
        ...
        <div v-else class="min-h-0 flex-1 overflow-y-auto py-2">
          ...
          <TabPanel :model-value="selectedTab" value="sjsx">
            <AllNodesPanel
              v-model:expanded-keys="expandedKeys"
              :sections="renderedSjsxSections"
...
const sjsxSections = computed(() => {
  if (selectedTab.value !== 'sjsx') return []
  return nodeOrganizationService.organizeNodesByTab(activeNodes.value, 'sjsx')
})

const renderedSjsxSections = computed(() => renderSections(sjsxSections.value))
...
async function handleSearch() {
  ...
  if (selectedTab.value === 'sjsx') {
    for (const section of sjsxSections.value) {
      allKeys.push(...collectFolderKeys(section.tree))
    }
  } else if (selectedTab.value === 'essentials') {
...
const tabs = computed(() => {
  const allTabs: Array<{ value: TabId; label: string }> = [
    { value: 'sjsx', label: t('sideToolbar.nodeLibraryTab.sjsx') },
