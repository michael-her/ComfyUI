...
import { getSjsxActiveParams, removeSjsxParam } from '@/utils/sjsxParamUtil'
...
const isSjsxNode = computed(() =>
  isSjsxNodeType(node.type || node.constructor?.comfyClass)
)

const canDeleteSjsxParam = computed(
  () => isSjsxNode.value && getSjsxActiveParams(node).includes(widget.name)
)
...
function handleDeleteSjsxParam() {
  removeSjsxParam(node, widget.name)
  canvasStore.canvas?.setDirty(true, true)
}
</script>
...
<template>
  ...
  >
    <template #default="{ close }">
      ...
      <Button
        v-if="canDeleteSjsxParam"
        variant="textonly"
        size="unset"
        class="flex w-full items-center gap-2 rounded-sm px-3 py-2 text-sm text-destructive transition-all hover:text-destructive active:scale-95"
        @click="
          () => {
            handleDeleteSjsxParam()
            close()
          }
        "
      >
